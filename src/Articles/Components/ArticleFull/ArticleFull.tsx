...
export function ArticleFull() {
    ...
    const handleSave = async () => {
        ...
        if (article.id === -1) {
            res = await Requester.articleCreatePrivate(
                newArticle,
                user.access_token
            );
            command = 'add';
    ...
    const handlePublicSave = async () => {
        ...
        if (newArticle) {
            const res = await Requester.articleValidate(
                newArticle.id,
                user.access_token
            );
    ...
    const handleDelete = async () => {
        const res = await Requester.articleDelete(
            article.id,
            user.access_token
        );
        res.id = article.id;
    ...
    const handleSubmit = async () => {
        const res = await Requester.articleSubmit(
            article.id,
            user.access_token
        );
        res.id = article.id;
    ...
    const isSavable =
        (isUpdatable || article.id === -1) && inModif && isValid && !inDelete;
    const isPublicSavable =
        isSavable && isAuthor && article.status !== 'public';
    const isDeletable = (isOwner || isModo) && !inModif;
    const isSubmitable =
        isOwner && !inModif && article.status === 'private' && !inDelete;
    const isPublishable =
    ...
    return (
        <div className={BootStrap.ARTICLE}>
            {((article.id === -1 && user.access_token !== '') ||
                article.id > 0) && (
                <div>
                    {!isValid && inModif && (
                                    ...
                                    <span>
                                        {!inDelete && (
                                            <button
                                                ...
                                                className={BootStrap.BAD_BUTTON}
                                            >
                                                Supprimer
                                            </button>
                                        )}
                    ...
                    {inModif && (
                        <div>
                            ...
                            <DropDownPublicArticles
                                ...
                                setValue={(value) =>
                                    handleSelections('requirements', value)
                                }
